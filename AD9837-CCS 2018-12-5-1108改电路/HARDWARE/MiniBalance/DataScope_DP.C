...
void SendDisplay()
{
    unsigned char n;          //计数变量
    unsigned char Send_Count; //串口需要发送的数据个数 
    
	extern uint16_t ADC_buff[300];
    extern uint16_t i;
 //   uint8_t j ;    
//  DataScope_Get_Channel_Data((float)ADC_buff[j], 1 ); 
    DataScope_Get_Channel_Data((float)ADC_buff[i], 1 );     
//	DataScope_Get_Channel_Data((float)temp_DHT11, 2 ); 
//	DataScope_Get_Channel_Data((float)temp_18B20, 3 );  
...
//  DataScope_Get_Channel_Data(Value9, 10); 
    Send_Count = DataScope_Data_Generate(10); //生成1个通道的 格式化帧数据，返回帧数据长度     
	for( n = 0 ; n < Send_Count; n++)  //循环发送,直到发送完毕   
	{  
       // printf("11");
        printf("%d",DataScope_OutPut_Buffer[n]);
       // delay_ms(60);
