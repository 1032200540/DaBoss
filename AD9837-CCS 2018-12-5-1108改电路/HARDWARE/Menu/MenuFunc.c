...
但这样就要重新另外写类似BarMenuUp等函数来维护显示
*/
/*
这里使用一个仅仅跟LCD显示条项数目一致的缓冲区来缓冲动态生成的条项
当实际条项数目MaxDynamicItem<=DispMax时 函数BarMenuUp BarMenuDown工作正常
...
//    GUI_Clear();
    ...
    if(MaxDynamicItem >DispMax){//需要显示的项数比液晶可以显示的多
        ItemNum = DispMax;
        //08/29
        BarDataMode = DynamicData;//通知BarMenuUp和BarMenuDown函数现在的数据是动态数据
