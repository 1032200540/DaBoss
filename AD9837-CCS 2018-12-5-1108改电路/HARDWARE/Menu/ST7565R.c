...
void LCD_IOinit(void)
{
	...
	RCC_APB2PeriphClockCmd(RCC_APB2Periph_GPIOF|RCC_APB2Periph_GPIOC, ENABLE);  //使能PF,PC 端口时钟  

	GPIO_InitStructure.GPIO_Pin = GPIO_Pin_0;           
	GPIO_InitStructure.GPIO_Mode = GPIO_Mode_Out_PP;    //推挽输出 
	GPIO_InitStructure.GPIO_Speed = GPIO_Speed_2MHz; 
	GPIO_Init(GPIOF, &GPIO_InitStructure); 
	GPIO_SetBits(GPIOF,GPIO_Pin_0);              //LCM_DB0输出高 
 
	GPIO_InitStructure.GPIO_Pin = GPIO_Pin_1;       
	GPIO_Init(GPIOF, &GPIO_InitStructure);   
	GPIO_SetBits(GPIOF,GPIO_Pin_1);              //LCM_DB1输出高 

	GPIO_InitStructure.GPIO_Pin = GPIO_Pin_2;      
	GPIO_Init(GPIOF, &GPIO_InitStructure);   
	GPIO_SetBits(GPIOF,GPIO_Pin_2);               //LCM_DB2输出高   

	GPIO_InitStructure.GPIO_Pin = GPIO_Pin_3;      
	GPIO_Init(GPIOF, &GPIO_InitStructure);   
	GPIO_SetBits(GPIOF,GPIO_Pin_3);               //LCM_DB3输出高  

	GPIO_InitStructure.GPIO_Pin = GPIO_Pin_4;       
	GPIO_Init(GPIOF, &GPIO_InitStructure);   
	GPIO_SetBits(GPIOF,GPIO_Pin_4);               //LCM_DB4输出高  

	
	GPIO_InitStructure.GPIO_Pin = GPIO_Pin_5;       
	GPIO_Init(GPIOF, &GPIO_InitStructure);   
	GPIO_SetBits(GPIOF,GPIO_Pin_5);               //LCM_DB5输出高  

	
	GPIO_InitStructure.GPIO_Pin = GPIO_Pin_6;       
	GPIO_Init(GPIOF, &GPIO_InitStructure);   
	GPIO_SetBits(GPIOF,GPIO_Pin_6);               //LCM_DB6输出高  
	
	GPIO_InitStructure.GPIO_Pin = GPIO_Pin_7;       
	GPIO_Init(GPIOF, &GPIO_InitStructure);   
	GPIO_SetBits(GPIOF,GPIO_Pin_7);               //LCM_DB7输出高 

	GPIO_InitStructure.GPIO_Pin = GPIO_Pin_0;        
	GPIO_Init(GPIOC, &GPIO_InitStructure);   
	GPIO_SetBits(GPIOC,GPIO_Pin_0);               //LCM_RS输出高  

	GPIO_InitStructure.GPIO_Pin = GPIO_Pin_10;       
	GPIO_Init(GPIOF, &GPIO_InitStructure);   
	GPIO_SetBits(GPIOF,GPIO_Pin_10);               //LCM_RESET输出高  

	GPIO_InitStructure.GPIO_Pin = GPIO_Pin_2;        
	GPIO_Init(GPIOC, &GPIO_InitStructure);   
	GPIO_SetBits(GPIOC,GPIO_Pin_2);               //LCM_CS输出高  

	GPIO_InitStructure.GPIO_Pin = GPIO_Pin_9;       
	GPIO_Init(GPIOF, &GPIO_InitStructure);   
	GPIO_SetBits(GPIOF,GPIO_Pin_9);               //LCM_RD输出高 

	GPIO_InitStructure.GPIO_Pin = GPIO_Pin_8;        
	GPIO_Init(GPIOF, &GPIO_InitStructure);   
	GPIO_SetBits(GPIOF,GPIO_Pin_8);               //LCM_WR输出高 

	
	GPIO_InitStructure.GPIO_Pin = GPIO_Pin_1;		 
	GPIO_Init(GPIOC, &GPIO_InitStructure);	 
	GPIO_SetBits(GPIOC,GPIO_Pin_1); 			  //背光BLKA输出高，开启 
	
}
	
	
//12864写数据
void LCD_WDATA(u8 data1)
{
	
    if(data1&0x01){LCM_DB0=1;} else LCM_DB0=0;
	if(data1&0x02){LCM_DB1=1;} else LCM_DB1=0;
	if(data1&0x04){LCM_DB2=1;} else LCM_DB2=0;
	if(data1&0x08){LCM_DB3=1;} else LCM_DB3=0;
	if(data1&0x10){LCM_DB4=1;} else LCM_DB4=0;
	if(data1&0x20){LCM_DB5=1;} else LCM_DB5=0;
	if(data1&0x40){LCM_DB6=1;} else LCM_DB6=0;
	if(data1&0x80){LCM_DB7=1;} else LCM_DB7=0;
}
		

/*
*****************************************************************************
* WriteCommand_ST7565R - 写命令到ST7565R
* DESCRIPTION: -
* 本函数不接管片选信号
* @Param CmdData:命令码
* @Return :
...
*****************************************************************************
*/
void WriteCommand_ST7565R(U8 CmdData)
{
     LCM_CS=0;  //片选=0，选中此 IC  
	 LCM_RS=0;  //RS=0：表示以下传输的一个字节是命令 
	 LCM_RD=0;  //
	 LCM_WR=0;  //RW=0:写 
	 LCD_WDATA(CmdData);   
	 LCM_RD=1;   
	 delay_us(2); 
	 LCM_CS=1;  //传完数据后片选=1，防止意外选中此 IC，对抗干扰有好处 
	 LCM_RD=0; 
...
{
	 LCM_CS=0;  //片选=0，选中此 IC  
	 LCM_RS=1;  //RS=1：表示以下传输的一个字节是要显示的点阵数据 
	 LCM_RD=0;  //E:读写使能 
	 LCM_WR=0;  //RW=0:写 
	 LCD_WDATA(Data);  
	 LCM_RD=1; 
...
/*
    ...
    PIN_ST7565R_RW_HIGH;
    DelayS(0);
    PIN_ST7565R_Enable_HIGH;
    DelayS(0);
    PIN_ST7565R_Enable_LOW;
    Temp = PIN_ST7565R_Lcd_InBus;
    return (Temp);
*/
}
...
//Option = 1   --->Disp On
//Option = 0   --->Disp Off
void ST7565R_DisplayOn(U8 Option)
{
    if (Option == 1) 
	{
        WriteCommand_ST7565R(ST7565R_CMD_Disp_ON);
    }
	else
	{
        WriteCommand_ST7565R(ST7565R_CMD_Disp_OFF);
    }
}

void ST7565R_SetStartRow(U8 Row)
{
    WriteCommand_ST7565R(ST7565R_CMD_Disp_Start_Line + Row);
}
/*
*****************************************************************************
...
*****************************************************************************
*/
void Display_Locate(U8 DisplayData,  u8 column, u8 page)
{
   	lcd_address( page, column);
    WriteData_ST7565R(DisplayData);
...
{ 
	 ...
	 WriteCommand_ST7565R(0xb0+page);        //设置页地址。每页是 8 行。一个画面的64 行被分成8 个页。 
	 WriteCommand_ST7565R(((column>>4)&0x0f)+0x10); //设置列地址的高 4 位 
	 WriteCommand_ST7565R(column&0x0f);     //设置列地址的低 4 位 
} 
...
{
	unsigned char i,j; 
	 for(i=0;i<9;i++)   //液晶驱动IC有65 行（分为9个页） ，只引出来64 行（8 页） ，有1 行没引出来，清屏时一并清掉。 
	 { 
		   lcd_address(1+i,1); 
		   for(j=0;j<132;j++)  //液晶驱动IC有132 列，只引出来128 列，有 4列没引出来，所以清屏时一并清掉。 
		   { 
				WriteData_ST7565R(0x00);//全部显示数据是0，以此清屏。 
	       } 
	 } 
}
...
void Display_Init(void)
{
	...
	LCM_RESET=1;	  //复位完毕 
	delay_us(20);		  
	WriteCommand_ST7565R(0xe2);  //软复位 
	delay_us(50); 
	WriteCommand_ST7565R(0x2c);  //升压步聚1：打开升压 
	delay_us(50);  
	WriteCommand_ST7565R(0x2e);  //升压步聚2：打开升压及电压调整器 
	delay_us(50); 
	WriteCommand_ST7565R(0x2f);  //升压步聚3：打开升压、电压调整器、电压跟随器 
	delay_us(50); 
	WriteCommand_ST7565R(0x24);  //粗调对比度，可设置范围 0x20～0x27 
	WriteCommand_ST7565R(0x81);  //微调对比度 
	WriteCommand_ST7565R(0x1a);  //0x1A,微调对比度的值，可设置范围0x00～0x3f 
	WriteCommand_ST7565R(0xa2);  //1/9偏压比（bias） 
	WriteCommand_ST7565R(0xc0);  //行扫描顺序：从上到下 
	WriteCommand_ST7565R(0xc8);  //列扫描顺序：从左到右 
	WriteCommand_ST7565R(0x60);  //扫描起始行：0X40 表示从第1 行开始，0x41:第2 行...0x60:第33 行，由于 LCD 的走线设计，本LCD 特殊地从 60 行开始扫描。 
	WriteCommand_ST7565R(0xaf);  //打开显示 
}
...
*****************************************************************************
*/
//void GUI_DispStringAt(U8 const*s,U8 x0,U8 y0)
void GUI_DispStringAt(U8 *s,U8 x0,U8 y0)
{
    ...
    while(s[i]!='\0')
	{
		...
		{//是中文
            ...
            Display_Chinese(s[i]*256+s[i+1],x0,y0);//中文。一个汉字有2个字节构成，*256表示前一个字节左移8位
            i++;
            x0 += Chinese_XSIZE;
       }
       s++;
...
//-------------显示普通图片
//------(图像 长度 起始位置)
void Display_Image(U8 const*IMG)
{
    U8 XAddr;
    U8 YAddr;
    U16 Count;
...
U8 const BarCode0[]={0xFF,0xFE,0xFC,0xF8,0xF0,0xE0,0xC0,0x80,0x00};
U8 const BarCode1[]={0x00,0x01,0x03,0x07,0x0F,0x1F,0x3F,0x7F,0xFF};
