...
you are using.
此示例提供了如何使用EVAL_COM1发送和接收中断与超级终端进行通信的说明.EVAL_COM1可以是USART1或USART2，具体取决于您使用的STMicroelectronics EVAL板。

First, the EVAL_COM1 sends the TxBuffer to the hyperterminal and still waiting for a string from the hyperterminal that you must enter.
首先，EVAL_COM1将TxBuffer发送到超级终端并仍在等待您必须输入的超级终端的字符串。
The string that you have entered is stored in the RxBuffer array. The receive buffer have a RxBufferSize bytes as maximum (the reception is stopped when this maximum receive value is reached).
您输入的字符串存储在RxBuffer数组中。 接收缓冲区的RxBufferSize字节为最大值（当达到此最大接收值时，接收停止）。
