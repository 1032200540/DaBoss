...
此示例使用DMA功能，标志和中断提供USARTy和USARTz之间的基本通信。 
 USART和USARTz可以是USART1和USART2或USART2和USART3，
具体取决于您使用的STMicroelectronics EVAL板。

首先，DMA将数据从TxBuffer2缓冲区传输到USARTz传输数据寄存器，
然后将此数据发送到USARTy。  USARTy接收的数据使用RXNE标志传输并存储在RxBuffer1中，
然后与发送的数据进行比较，比较结果存储在“TransferStatus1”变量中。
 
同时，DMA将数据从TxBuffer1缓冲区传输到USARTy传输数据寄存器，
然后将此数据发送到USARTz。  USARTz接收的数据使用接收中断传输并存储在RxBuffer2中，
然后与发送的数据进行比较，比较结果存储在“TransferStatus2”变量中。
