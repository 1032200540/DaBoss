...
此示例提供了如何使用SPI固件库和关联SPI FLASH驱动程序与M25P64或M25P128 FLASH通信的基本示例。
...
第一步是读取SPI Flash ID。 完成从SPI闪存读取的ID与预期的ID之间的比较，并且在成功的情况下LED1接通，否则LED2接通。
...
使用该驱动程序，程序执行擦除要访问的扇区，将main.c文件中定义的Tx_Buffer写入存储器，然后读取写入的数据。 
然后将从存储在Rx_Buffer中的存储器读取的数据与Tx_Buffer的期望值进行比较。 此比较的结果存储在“TransferStatus1”变量中。
