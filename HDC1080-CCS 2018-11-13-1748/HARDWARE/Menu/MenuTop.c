...
void InitMenuVal(void)
{
    ...
    ItemBackup_i = 0;
    Layer = 0;
    FatherIndexBackup[Layer++] = MenuTopIndex;  //push index  待机界面作为起点
}
/*
*****************************************************************************
* DispHomeScreen - 待机界面层5个函数
* DESCRIPTION: -
...
{
    GUI_Clear();
	GUI_DispStringAtBar("温湿度检测",4,1,124,GUI_TA_HCENTER);
...
void PUSH(void)
{
    WhereReverseBackup[WhereBackup_i++] = Where;      //备份Where--->高亮显示的位置
    ItemBackup[ItemBackup_i++] = Item;         //备份Item--->当前项的位置
    FatherIndexBackup[Layer++] = KeyFuncIndex;      //备份KeyFuncIndex--->状态机的状态
//-------------------------------
...
void POP(void)
{
    ...
    KeyFuncIndex = FatherIndexBackup[Layer-1];     //恢复KeyFuncIndex
    Where = WhereReverseBackup[--WhereBackup_i];    //恢复高亮显示位置Where
    Item = ItemBackup[--ItemBackup_i];       //恢复项位置Item
//-------------------------------

//-------------------------------
    Refresh = FlashMode_ReFlash;                     //子项返回,请求刷新
}
...
//-------------------
    ...
    GUI_DispStringAtBar(*(DispItem+Item),1,ICO_PosY+ICO_YSize,126,GUI_TA_HCENTER);//菜单名字
